...
echo $((pi=22/7,bpi=335/113,bpi-pi)) # returnwaarde van laatste zal hij geven hier het verschil
...
# grootste da je kan done is 64 da is alles daarnet (kleine en grote letter) + @ en _
...
set -- $(cat tel) # altijd als je gebruik maakt van set dan vertrek je vanaf nul dus de rest is hier nu allemaal weg
...
shift 3 ; echo ${@} # zal altijd shift 3 doen tot hij niemeer kan EN DAN DOE HEM NIETS de stommerik
# bvb zit enkel tien in dan doe je shift 3 dan blijft er tien in zitten

shift 3 ; echo $? #foutcode is $?
# dus eerste paar keer 0
# laatste keer krijg je 1 dus hem is nie gebeurd 


set -- ${@:-...} # ookal is er niets opgegeven van parameters doe alsof dit de parameters zijn
...
t=([2]=kat [7]=hond) # je definieert de tabel volledig opnieuw
# wil je dat de oude bewaard blijven dan moet je de += notatie gebruiken

t+=([2]=kat [7]=hond) # EXAMEN
declare -p t

t+=(vogel vis) # ik ga tabel wijzigen en behou alles wat ik had dus nu ga je achteraan toevoegen

t+=($(cat lees)) # let op het verschil tss de soorten () () de een is command substitution de andere tabelwijziging
...
# nee je krijgt het aantal effectief ingevulde elementen !! (niet laatste index eh!)
...
echo ${t[@]: 8:1} # beer 
# je krijgt eerste element waarvan index hoger of gelijk is aan wat je opgegeven heeft hier is [8]=beer
# M -> zinloos
...
unset t 
read -a t < competitors.csv # je gaat het dus niet in variabele inlezen maar in een tabel 
...
grep -E \([aeiouy][aeiouy].*\){4} dido # vier keer opeenvolgende klinker op de lijn
