...
lspci -nv | grep -e "^0" -e driver # udev heeft dit dynamisch geladen
...
echo -n 0000:00:0b.0 > unbind # -n voor geen scheidingsteken plaatsen in unbind

lsblk -f # heel die usb controller is weg 

echo -n 0000:00:0b.0 > bind # nu plug je de usb controller in zijn geheel weer in 
...
udevadm test -a add|remove /sys/... # simuleren wat het effect zou zijn als je opnieuw zou inpluggen
...
mount -t devpt -o gid=5,mode=620 devpts /dev/pts # da haalt hem allemaal uit etc/fstab uit de verschillende kolommen
# bij de rest staat optie default (geen opties)

# de nummerkes geven aan in welke volgorde ze moeten uitgevoerd worden (eerst nen root en dan pas zijn kindjes want anders gaat dat niet)
...
# mount -o hoeveelgeheugenwiljereserveren -t tmpfs [device] [doel] # die eerste parameter device is nie echt logisch maja
...
mount -o size=800m,remount -t tmpfs tmpfs mem # EXAMEN!!!!!
...
# in fases waar de /etc/mtab nog read only is moet je de mount -n gebruiken (want het kan niet normaal gemount worden zogezegd)

# INTERESSANT BIND optie

# chroot jails -> afgesloten omgeving voor specifiek proces (apart filesysteem die maar aan beperkte dingen kan)
...
cp /usr/bin/bash /root/jail/usr/bin/bash # goed idee? nee

ln /usr/bin/bash /root/jail/usr/bin/bash # hardlink!

stat /usr/bin/bash /root/jail/usr/bin/bash # zelfde inode hier 1861
# zelfde fileke komt op verschillende plaatsen voor binnen hetzelfde filesysteem
...
ls -lad /lib/libtinfo.so.5 # ma da is een symbolische link naar een ander bestand
...
cp /usr/lib/libtinfo.so.5 . # dan ebde weer veel kopies -> weer hardlinks
...
stat /usr/lib/libtinfo.so.5 libtinfo.so.5 # niet dezelfde bestandjes, andere inodes!!
# want twas nen symlink naar dienen 9 hier
stat /usr/lib/libtinfo.so.9 libtinfo.so.5 # wel dezelfde inode dus wel dezelfde bestandjes!!
# moest het op een ander filesysteem staan dan had hem geen hardlink kunnen maken 
...
chroot /root/jail # zou bash moeten oproepe ma doe da nie 
...
# rm -rf * # bij sym links en hard links geen probleem maar bij die mount OEI OEI OEI 
